...
def create_verified_test_profile(service: EffectBrowserService):
    settings = get_settings()
    profile = service.store.create_profile(
        tenant_id=settings.default_tenant_id,
        name="Synthetic reactive test profile",
    )
    values = {
        "full_name": "Synthetic Reactive Candidate",
        "email": "reactive@example.test",
        "country": "Ireland",
        "work_authorization": "authorized",
        "years_using_python": "6",
        "resume_summary": (
            "Synthetic profile with production Python, distributed systems, "
            "durable execution, and observability experience."
        ),
...
@pytest.mark.e2e
def test_missing_verified_identity_fact_stops_before_model_can_fill_it(
    tmp_path,
    monkeypatch,
) -> None:
    base_url, server, thread = start_harness(tmp_path, monkeypatch)
    try:
        ...
        task = service.create_task(
            tenant_id=settings.default_tenant_id,
            instruction="Prepare a synthetic application without guessing identity.",
        ...
        assert result.task.status.value == "awaiting_input"
